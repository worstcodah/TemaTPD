...
<script>
    function initMap() {
        ...
        var map = new google.maps.Map(document.getElementById('map'), {
            center: { lat: <%=request.getSession().getAttribute("latitude")%>, lng: <%=request.getSession().getAttribute("longitude")%> },
            zoom: 12 // Adjust the zoom level as needed
        });

        // Create a marker on the specified coordinates
        var marker = new google.maps.Marker({
            position: { lat: <%=(double)request.getSession().getAttribute("latitude")%>, lng: <%=(double)request.getSession().getAttribute("longitude")%> },
            map: map
        });
    }
    ...
    function loadScript() {
        ...
        script.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyD-Ru_J6VDZfXcow9nEP7aJ_ACoRiw4Kgc&callback=initMap';
